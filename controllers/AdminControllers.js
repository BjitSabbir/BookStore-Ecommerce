        ...
        try {
            if (req.user.role === 1) {
                ...
                if (!errorQuery.isEmpty()) {
                    return res.status(NOT_FOUND).send(errorMessage(errorQuery.array()[0].msg));
                }

                const { search, page, limit, sortType, sortKey, userId } = req.query;
                ...
                const transactions = await TransectionModel.find(query, null, options).populate({
                    path: "books.bookId",
                    select: "title isbn",
                }).populate({
                    path: "userId",
                ...
                res.status(OK).send(successMessage("Welcome to BookStore Ecommerce Admin", transactions));
            } else {
                res.status(FORBIDDEN).send(errorMessage("User not authorized"));
            }
        } catch (error) {
            console.error(error);
            res.status(INTERNAL_SERVER_ERROR).send(errorMessage("Internal Server Error"));
        }
    }
        ...
        try {
            if (req.user.role === 1) {
                ...
                if (!mongoose.Types.ObjectId.isValid(userId)) {
                    return res.status(BAD_REQUEST).send(errorMessage("Invalid userId"));
                }
                ...
                if (user) {
                    res.status(OK).send(successMessage("Welcome to BookStore Ecommerce Admin", user));
                } else {
    ...
    async getAllUsers(req, res) {
        try {
            if (req.user.role === 1) { 
                ...
                res.status(OK).send(successMessage("User list retrieved successfully", users));
            } else {
                res.status(FORBIDDEN).send(errorMessage("User not authorized"));
            }
        } catch (error) {
    ...
    async editUserData(req, res) {
        try {
            if (req.user.role === 1) {
                ...
                const { username, email, isActiveUser, address, phone } = req.body;
                ...
                if (email && email !== userToUpdate.email) {
                    ...
                    if (emailExists) {
                        return res.status(BAD_REQUEST).send(errorMessage("Email already exists"));
                    }
                    ...
                    const authUser = await AuthModel.findOne({ userId: userToUpdate._id });
                ...
                res.status(OK).send(successMessage("User data updated successfully", updatedUser));
            } else {
                res.status(FORBIDDEN).send(errorMessage("User not authorized"));
            }
        } catch (error) {
        ...
        try {
            if (req.user.role === 1) {
                ...
                for (const review of reviews) {
                    ...
                    await BookModel.findByIdAndUpdate(bookId, { $pull: { reviews: review._id } });
                ...
                res.status(OK).send(successMessage("User deleted successfully"));
